...
module.exports.orchestrator = async (event) => {
            ...
            if (service.dependency.length === 0 && service.status !== Constants.COMPLETED) {
                ...
                dependencyServices.forEach(dep => {
                    const depFiltered = services.filter(item => (
                        dep === item.name && item.status === Constants.COMPLETED && item.modules.includes(MODULE_NAME)
                    ))
                    dependencyCompleted += depFiltered.length;
                    console.log(`==DEP SERVICES: \nDEP: ${dep} \nDEP FILTERED: ${JSON.stringify(depFiltered)} \nDEP COMPLETED: ${dependencyCompleted}`);
                })
                ...
                if (dependencyCompleted === service.dependency.length) {
                    ...
                    console.log(`== Dep completed: ${dependencyCompleted} Dep length: ${service.dependency.length}`)
                }
            }
            else {
    ...
    async function updateState(event) {
        const message = JSON.parse(event.Records[0].Sns.Message);
        console.log(`++ ${message.service_name}:  ${message}`);
        ...
        const service_payload = _.omit(message, ['_id', 'service_name']);
        ...
        try {
            ...
            await Dynamo.putData(RISK_ORCHESTRATOR_TABLE, payload);
            return payload;
        } catch (error) {
            console.log(error);
        }
    ...
    async function fetchLatestState(id) {
        try {
            return Dynamo.getData(RISK_ORCHESTRATOR_TABLE, '_id', id);
        }
        catch (err) {
